...
**Status:** partial qualification. The campaign ran to completion. 21 route
classes are Approved (3 carried over from WHI-557, 18 new). Every other bucket
variant of every 2- and 3-hop topology over {v2, v3, moe} is **explicitly
Unsupported with a reason**. Over the committed universe the profile gives zero
`UnknownRoute`. No class was approved by extrapolation, and no class was approved
to make a test pass. A human merge gate applies because this is the mainnet gas
profile.

## What changed in the profile

| | before (WHI-557, `0xa18811da…`) | after (this campaign) |
|---|---:|---:|
| profile entries | 24 | 307 (every 2..3-hop bucket variant plus the old h4 entry) |
| Approved | 3 | 21 (the 3 WHI-557 classes stay Approved; `h2:v2+v3:ticks=0` gas_limit goes from 328088 to 335569 because campaign samples were added) |
| `content_digest` | `0xa18811daf4a27e071575db61de1cb57751fb619378f754188c569c4144c1ccba` | `0x6d3daa2f26df7aad34a045945a5cc978f8982bb6f96f7d9a5e6da15fa8997799` |
...
is now `0x6d3daa2f…7799`. Executor identity is unchanged (`executor_code_hash`
...
- **Tables** below: `python3 evidence/gas/whi-1422/make_tables.py`.
...
What is **not** canonical: a real same-block round trip always loses to fees, so
the executor's on-chain profit check would revert every honest sample. Each sample
therefore uses one **settlement lever**, a state override on the fork:
...
lever-induced state. Gas depends on the storage slots touched and on the crossing
count, not on reserve magnitudes, and the lever keeps both close to real. It is
still not a replay of a real profitable transaction. Treat the numbers as
fork-measured and lever-assisted, not as mainnet receipts.
...
(WMNT-settled, 2..3 hops, all 130 rows). With the new profile, the topologies that
have an Approved variant cover **2490 of 6962 cycles (35.8%)**. The biggest
...
qualification samples: 0 < min_samples 10" or with the open-ended reason.
...
| `h2:moe+v3:ticks=0:bins=0` | 11 | 0 | 11 | 2 / 397483 (pass) | 526918 | 397431 | **Approved** |
...
| `h2:v3+moe:ticks=0:bins=0` | 18 | 0 | 18 | 3 / 361828 (pass) | 484088 | 335344 | **Approved** |
| `h2:v3+v2:ticks=0` | 9 | 0 | 15 | 3 / 237574 (pass) | 335272 | 237544 | **Approved** |
...
| `h2:v3+v3:ticks=0` | 23 | 0 | 29 | 5 / 289524 (pass) | 378496 | 267074 | **Approved** |
...
| `h3:moe+moe+v3:ticks=0:bins=0` | 14 | 0 | 14 | 2 / 506405 (pass) | 657658 | 499703 | **Approved** |
...
| `h3:moe+v3+v2:ticks=0:bins=0` | 12 | 0 | 12 | 2 / 492586 (pass) | 641104 | 487459 | **Approved** |
| `h3:moe+v3+v2:ticks=1-5:bins=0` | 2 | 0 | 0 | - | - | - | Unsupported: insufficient qualification samples: 2 < min_samples 10 |
| `h3:moe+v3+v3:ticks=0:bins=0` | 12 | 0 | 12 | 2 / 515025 (pass) | 666781 | 508588 | **Approved** |
...
| `h3:v2+moe+v3:ticks=0:bins=0` | 10 | 0 | 10 | 2 / 453877 (pass) | 593465 | 447127 | **Approved** |
...
| `h3:v2+v3+moe:ticks=0:bins=0` | 15 | 0 | 15 | 3 / 494166 (pass) | 636859 | 457091 | **Approved** |
...
| `h3:v2+v3+v3:ticks=0` | 19 | 2 | 19 | 3 / 369488 (pass) | 490703 | 357673 | **Approved** |
...
| `h3:v3+moe+moe:ticks=0:bins=0` | 12 | 0 | 12 | 2 / 594067 (pass) | 733774 | 386251 | **Approved** |
| `h3:v3+moe+v2:ticks=0:bins=0` | 17 | 2 | 17 | 3 / 451540 (pass) | 583770 | 352397 | **Approved** |
...
| `h3:v3+moe+v3:ticks=0:bins=0` | 16 | 0 | 16 | 3 / 458941 (pass) | 600662 | 457337 | **Approved** |
...
| `h3:v3+v2+v2:ticks=0` | 18 | 0 | 18 | 3 / 353128 (pass) | 473675 | 346396 | **Approved** |
...
| `h3:v3+v2+v3:ticks=0` | 12 | 0 | 12 | 2 / 377920 (pass) | 492707 | 356143 | **Approved** |
...
| `h3:v3+v3+v2:ticks=0` | 21 | 2 | 21 | 4 / 369071 (pass) | 483574 | 356934 | **Approved** |
...
| topology | variants | approved | unsupported (open-ended) | unsupported (other) |
|---|---:|---:|---:|---:|
| `h2:v2+v2` | 1 | 1 | 0 | 0 |
| `h2:v2+v3` | 4 | 1 | 1 | 2 |
| `h2:v3+v2` | 4 | 1 | 1 | 2 |
...
| `h3:v3+v3+v2` | 4 | 1 | 1 | 2 |
| `h3:v3+v3+v3` | 4 | 0 | 1 | 3 |
| `h3:moe+v2+v2` | 4 | 0 | 1 | 3 |
| `h3:moe+v2+v3` | 16 | 0 | 7 | 9 |
| `h3:moe+v3+v2` | 16 | 1 | 7 | 8 |
| `h3:moe+v3+v3` | 16 | 1 | 7 | 8 |
| `h3:v2+moe+v2` | 4 | 0 | 1 | 3 |
| `h3:v2+moe+v3` | 16 | 1 | 7 | 8 |
| `h3:v2+v2+moe` | 4 | 0 | 1 | 3 |
| `h3:v2+v3+moe` | 16 | 1 | 7 | 8 |
| `h3:v3+moe+v2` | 16 | 1 | 7 | 8 |
| `h3:v3+moe+v3` | 16 | 1 | 7 | 8 |
| `h3:v3+v2+moe` | 16 | 0 | 7 | 9 |
| `h3:v3+v3+moe` | 16 | 0 | 7 | 9 |
| `h3:moe+moe+v2` | 4 | 1 | 1 | 2 |
| `h3:moe+moe+v3` | 16 | 1 | 7 | 8 |
| `h3:moe+v2+moe` | 4 | 0 | 1 | 3 |
| `h3:moe+v3+moe` | 16 | 0 | 7 | 9 |
| `h3:v2+moe+moe` | 4 | 1 | 1 | 2 |
| `h3:v3+moe+moe` | 16 | 1 | 7 | 8 |
| `h3:moe+moe+moe` | 4 | 0 | 1 | 3 |
| `h4:v2+v2+v2+v2` | 1 | 0 | 0 | 1 |
...
- **`v3+v3+v3`, `v3+v3+moe`, `moe+v3+moe`, `moe+moe+moe`, `moe+moe`, and the
  2-V2 3-hops**: no Approved variant. See the tables for the reason per bucket.
