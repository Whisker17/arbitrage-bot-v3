...
## bot_action vocabulary (binary buckets + detail)

| bucket | meaning |
| --- | --- |
| **seed_no_amm_code** | Structure is drop-in for an existing family; universe/factory seed can list it. May still need **identity / fee wiring** (not new AMM math). |
| **needs_adapter** | Broken layer requires new math/events/accessor path before any seed. |
| **ignore** | Do not seed and do not plan an adapter now. |

`bot_action` cells use `bucket — detail`.

## Matrix

| venue | family | verdict | factory_or_deployer | sample_pool | fee_notes | bot_action | evidence_path |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Agni V3 | univ3_cl | `drop_in_univ3_or_agni` | factory `0x25780dc8Fc3cfBD75F33bFDAB65e969b603b2035`; deployer `0xe9827B4E…` | `0x1858d52c…` / `0xeAfc4D6d…` | pool `fee()` uint24; Agni tiers include 2500; census wrongly tags `algebra` | **seed_no_amm_code** — first-class `agni-v3` already | `agni-v3/` |
| FusionX V2 | univ2_cpmm | `drop_in_univ2` | factory `0xE5020961fA51ffd3662CDf307dEf18F9a87Cce7c` | `0x3e5922cD…` | **fee = `200 / 100_000` (0.2%)** live + fixture; bot `V2_FEE = 300` (0.3%) → **fee-mismatch risk** | **seed_no_amm_code** — only after per-venue fee `200 / 100_000` is wired (do not seed under hard-coded 300); reclassify interim `agni-v2` | `fusionx-v2/` |
| FusionX V3 | univ3_cl | `drop_in_univ3_or_agni` | factory `0x530d2766D1988CC1c000C8b7d00334c14B69AD71`; deployer `0x8790c2C3…` | `0x262255f4…` (USDT/WMNT fee=500; top uncovered arb pool) | pool `fee()` uint24; `feeProtocol` uint32 width (222825800) already handled by existing V3 reader; census wrongly tags `algebra` | **seed_no_amm_code** — first-pass multi-factory V3 seed (identity only; do not merge CREATE2 into Agni) | `fusionx-v3/` |
| Butter | univ3_cl | `drop_in_univ3_or_agni` | factory `0xEECa0a86431A7B42ca2Ee5F479832c3D4a4c2644` (no `poolDeployer`) | `0x67f1e667…` (WMNT/WETH fee=500) | per-pool `fee()`; tiers 100/500/3000/10000 (`fee_and_getpool.txt`); no 2500 | **seed_no_amm_code** — first-pass multi-factory V3 seed | `butter/` |
| Merchant Moe LB | moe_lb | `drop_in_moe_lb` | factory `0xa6630671775c4EA2743840F9A5016dCf2A104054` | `0x2612E328…` / `0x1606c79b…` | LB bin-step / dynamic fees | **seed_no_amm_code** — first-class `moe` already | `moe-lb/` |
| Merchant Moe V1 (classic) | univ2_cpmm | `drop_in_univ2` | factory `0x5bEf015CA9424A7C07B68490616a4C1F094BEdEc` | `0x4E7685Df…` / `0x7c88dd67…` | fee = `300 / 100_000` (matches bot default) | **seed_no_amm_code** — optional later UniV2 | `moe-v1/` |
| Fluxion V3 | univ3_cl | `drop_in_univ3_or_agni` | factory `0xF883162Ed9c7E8EF604214c964c678E40c9B737C` | `0xB1C1df81…` / `0x361052be…` | UniV3 uint24 fee; tiers 500/3000/10000 | **seed_no_amm_code** — first-pass multi-factory V3 seed | `fluxion/` |
...
| Agni V2 (label) | — | **alias of FusionX V2** | same as FusionX V2 | same | see FusionX V2 | **ignore** as separate factory — alias of FusionX V2 | `agni-v2/` |
...
### Seed now — drop-in, no new AMM math (`seed_no_amm_code`)
...
| `fusionx-v3` | `0x530d2766D1988CC1c000C8b7d00334c14B69AD71` | `0x8790c2C3…` | **Highest missing V3 coverage**; multi-factory V3 identity only |
...
### Seed only after fee wiring (still no new AMM math)

| label | factory | notes |
| --- | --- | --- |
| `fusionx-v2` | `0xE5020961fA51ffd3662CDf307dEf18F9a87Cce7c` | Structure is drop-in UniV2; quotes wrong under bot `V2_FEE = 300`. Wire fee **`200 / 100_000`** (or per-pool fee) **before** seeding into any executable path. Interim `agni-v2` label should retire. |
...
### Coverage implication (from WHI-906 analysis, **not re-measured** in this issue)

WHI-906 estimated that adding all census-tagged drop-in V2/V3 venues would move fully-executable arbs from ~4.4 % toward a large majority of the 30d market, with iZi as a later adapter slice. **This issue only confirms the contract-level drop-in vs adapter split for the top factories by arb legs** — it does not re-run the coverage script.
