...
async fn main() -> eyre::Result<()> {
    ...
    // Mantle RPC URL
    let rpc = std::env::var("RPC_URL").unwrap_or_else(|_| "https://rpc.mantle.xyz".to_string());

    // Agni Factory on Mantle (provided by user)
    let factory_addr: Address = "0x0d922Fb1Bc191F64970ac40376643808b4B74Df9".parse()?;
    ...
    let creation_block_env = std::env::var("MANTLE_UNI_V3_FACTORY_DEPLOY_BLOCK").ok();
    let creation_block = creation_block_env
        .as_deref()
        .and_then(|s| s.parse::<u64>().ok())
        .unwrap_or(63795918);

    println!("[uni_v3:list] rpc={}", rpc);
    println!(
        "[uni_v3:list] factory={:?} creation_block={} ",
    ...
    let factory = UniswapV3Factory::new(factory_addr, creation_block);
    ...
    println!(
        "[uni_v3:list] discovered {} pools (unsynced)",
        pools_unsynced.len()
    );
    ...
    if do_sync {
        ...
        println!("[uni_v3:list] synced and kept {} pools", pools_synced.len());
