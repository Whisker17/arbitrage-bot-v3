//! Generate and optionally on-chain-validate `data/poolLists_moe.csv`.
//!
//! ```bash
//! # Discover from factory creation → head (slow: chunked eth_getLogs)
//! cargo run --example generate_moe_pool_list
//!
//! # Pin a canonical end block for deterministic regeneration
//! MOE_POOL_LIST_TO_BLOCK=98000000 cargo run --example generate_moe_pool_list
//!
//! # Skip on-chain token/factory re-check after write
...
async fn main() -> Result<()> {
    ...
    let to_block = std::env::var("MOE_POOL_LIST_TO_BLOCK")
        .ok()
    ...
    info!(path = %out_path.display(), "Wrote Moe pool list");
    ...
    // Determinism check: re-parse written file
    let reloaded = amms::amms::moe::MoePoolList::load_path(&out_path)
        .context("reload written list")?;
